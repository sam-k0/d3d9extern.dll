...
void CAviTexture::Update(float frameTime){ 
	...
	lpbi = (LPBITMAPINFOHEADER)AVIStreamGetFrame(m_pgf, m_frame);
	...
	D3DLOCKED_RECT rect;  m_pTexture->LockRect(0, &rect, NULL, 0);
	// Get DWORD pointers to dest and src data 
	DWORD* pDestData = (DWORD*)rect.pBits;  DWORD* pSrcData  = (DWORD*)m_pData; 
...
{
	//	if (SUCCEEDED(
//	pSprite->Draw(m_pTexture, NULL, &D3DXVECTOR3(0, 0, 0), &D3DXVECTOR3(x, y, 0), D3DCOLOR_RGBA(255, 255, 255, 255));
		//))
	/* {
			return true;
		}
